...
FULL JOIN departments d
ON e.department_id = d.department_id
WHERE  e.first_name || ' ' ||last_name = 'Steven King';
--다르게 풀면 !
SELECT
	d.department_name 
FROM employees e
...
--d.manager_id는 안되나????????
--밑에꺼 참고, 매니저 없고 사원 존재하는 경우의 수도 생각해봐야 한다. 
SELECT
d.department_name,
e.employee_id
FROM departments d
...
SELECT
    e1.first_name as 사원, e2.first_name as 매니저
FROM employees e1
JOIN employees e2
ON e1.manager_id = e2.employee_id;
/*
...
SELECT
    e1.first_name as 사원,
    e2.first_name as 매니저, e2.salary as 매니저월급
FROM employees e1 -->사원
