...
CREATE SEQUENCE dept2_seq
    ...
    --시퀀스가 CYCLE이면 10까지 간 뒤 초기화 되고 다시 1부터 시작
DROP TABLE dept2;
...
ALTER SEQUENCE dept2_seq MAXVALUE 9999; --9999까지 넣을 수 있음
ALTER SEQUENCE dept2_seq INCREMENT BY -1; -- -1씩 감소시킴
ALTER SEQUENCE dept2_seq MINVALUE 0;--최소값 변경

--시퀀스 값을 다시 처음으로 돌리는 방법
ALTER SEQUENCE dept2_seq INCREMENT BY -43;
...
테이블 전체조회가 아닌, 컬럼에 붙은 인덱스를 이용해서 조회를 진행합니다. 
...
/*
- 인덱스가 권장되는 경우 
1. 컬럼이 WHERE 또는 조인조건에서 자주 사용되는 경우 ->ex) primary key
