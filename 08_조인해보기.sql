...
*/
--employees 테이블의 부서 id와 일치하는 departments테이블의 부서id를
...
AND loc.state_province = 'California';--(1)일반조건
/*
1. location테이블의 state_province = 'California'조건에 맞는 값을 
대상으로
2. location_id 값과 같은 값을 가지는 데이터를 departments에서 찾아서 조인
3. 위에 조인한 결과와 동일한 department_id를 가진 employees테이블의 데이터를 찾아 조인
4. 위에 결과와 jobs 테이블을 비교하여 조인하고 최종결과를 출력.
일반조건부터 먼저 거르고 그 다음 테이블을 붙이는 식으로 실행된다.
*/

--외부 조인
/*
상호 테이블 간에 일치되는 값으로 연결되는 내부 조인과 다르게
...
(+)가 붙지 않은 테이블을 기준으로 departments 테이블이 조인에 
참여하라는 의미를 부여하기 위해 기호를 붙입니다.
e.department_id 를 기준으로 d.department_id값들이 붙는다. 
즉 e.department_id의 값은 무조건 전부가 나온다.

외부조인을 사용했더라도 이부에 내부 조인을 사용하면
내부 조인을 우선적으로 인식합니다.
*/
